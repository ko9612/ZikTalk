...
const QuestionBookmarkList = ({ testEmpty }) => {
  ...
  const toggleBookmark = (id) => {
    ...
    if (starredItems.includes(id)) {
      updated = starredItems.filter(bid => bid !== id);
    } else {
      updated = [...starredItems, id];
    }
  ...
  const bookmarks = testEmpty ? [] : faqData
    .filter(item => {
      // 1. 직무(career) 필터링
      const careerMatch = (job === '직군·직무') || (item.career === job);
      // 2. 기존 type '일반' 제외 필터링
  ...
  return (
    <div className="w-full max-w-5xl mx-auto py-8">
      {filtered.length === 0 ? (
        ...
        <>
          <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-center text-zik-text">질문 북마크</h2>
          {/* 필터 영역 */}
          <div className="flex gap-2 mb-4 items-center justify-between">
            <div className="flex gap-1 items-center">
              <button
                type="button"
                className="relative min-w-24 w-auto ml-1 flex items-center justify-center rounded-full border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-500 hover:bg-gray-50 focus:outline-none whitespace-nowrap truncate"
              ...
              <FilterDropdown
                ...
                options={typeOptions}
                className="relative min-w-12 w-auto  flex items-center justify-center rounded-full border border-gray-300 bg-white px-2 py-2.5 text-sm font-medium text-gray-500 hover:bg-gray-50 focus:outline-none whitespace-nowrap truncate"
              />
            </div>
            <button
              type="button"
              className="relative min-w-12 w-auto mr-2 ml-1 flex items-center justify-center rounded-full border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-500 hover:bg-gray-50 focus:outline-none whitespace-nowrap truncate"
              onClick={() => {
                setJob('직군·직무');
                setType('전체');
              }}
          ...
          {/* 표 헤더 */}
          <div className="hidden sm:grid grid-cols-12 items-center px-1 sm:px-2 md:px-4 py-2 border-t-2 border-t-gray-500 border-b-2 border-b-gray-200 text-gray-400 text-xs sm:text-sm md:text-base font-semibold tracking-wide mb-3">
            <div className="text-center pr-4 sm:pr-6 md:pr-14">No</div>
            <div className="col-span-0 text-center pr-4 sm:pr-6 md:pr-13">직무</div>
            <div className="col-span-1 text-center pl-4 sm:pl-6 md:pl-12">유형</div>
            <div className="col-span-8 text-left pl-4 sm:pl-6 md:pl-25">질문</div>
          ...
          {pageResults.map((item, idx) => (
            <div
              ...
              className="mb-3"
            >
              <div className="relative rounded-2xl mr-2 ml-1 border border-gray-300 bg-white shadow transition hover:shadow-lg overflow-hidden">
                ...
                <div className="flex flex-col sm:grid sm:grid-cols-12 items-center px-4 py-3 sm:py-4 gap-y-2 cursor-pointer" onClick={() => toggleOpen(item.id)}>
                  {/* badge row */}
                  <div className="flex min-w-0 items-center w-full sm:col-span-4">
                    <span className="text-xs  font-bold text-gray-400 sm:text-sm">{(page - 1) * PAGE_SIZE + idx + 1}</span>
                    {item.job && (
                  ...
                  {/* 질문 row */}
                  <div className="mt-1 mr-16 w-auto min-w-0 flex-1 text-sm font-medium break-words whitespace-normal text-gray-900 sm:mt-0 sm:ml-2 sm:col-span-8">
                    {item.question}
                  </div>
                </div>
                {/* 답변 아코디언: FaqItem 스타일로, 글자 크기/색상/여백 동일하게 */}
                <div
                  className={`overflow-hidden rounded-b-2xl bg-white px-4 transition-all duration-300 sm:px-6 ${openIds.includes(item.id)
                    ...
                    <>
                      {item.answer && (
                        <div className="mb-3 sm:mb-4">
                          <div className="mb-1 text-sm font-medium text-gray-500 sm:mb-2 sm:text-base">내 답변</div>
                      ...
                      {item.recommendation && (
                        <div className="mb-3 sm:mb-4">
                          <div className="mb-1 text-sm font-medium text-gray-500 sm:mb-2 sm:text-base">추천 답변</div>
                          <div className="rounded-2xl bg-gray-50 p-3 text-sm text-gray-800 sm:p-4 sm:text-base">
                            {item.recommendation}
                          </div>
                        </div>
