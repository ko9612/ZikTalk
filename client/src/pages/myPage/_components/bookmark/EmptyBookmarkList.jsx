import React from "react";
import { useNavigate } from "react-router-dom";
import { FiSearch } from "react-icons/fi";
import { TableHeader, TEXT_COLORS } from "./settings";
import Button from "@/components/common/Button";
const EmptyBookmarkList = () => {
  const navigate = useNavigate();

  return (
    <div className="mx-auto w-full ">
      <TableHeader />

      <div
        className={` flex min-h-[340px] flex-col items-center gap-2 sm:gap-5 mt-10 md:gap-5  ${TEXT_COLORS.normal}`}
      >
        <div className="flex h-20 w-20 items-center justify-center rounded-full bg-gray-100">
          <FiSearch className="text-5xl" />
        </div>
        <div className="flex flex-col items-center gap-1">
          <p className="font-semibold">북마크한 질문이 없습니다</p>
          <div className="text-center text-sm">
            <p>관심 있는 질문을 북마크하여 모아보세요.</p>
            <p>면접 준비에 도움이 됩니다.</p>
          </div>
        </div>
        <Button onClick={() => navigate("/mypage/result-list")}>질문 북마크하기</Button>
      </div>
    </div>
  );
};

export default EmptyBookmarkList;
