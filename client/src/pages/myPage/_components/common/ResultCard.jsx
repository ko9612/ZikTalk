...
export const ActionButton = ({
  ...
  return (
    <button
      ...
      className={`hover:text-zik-main ml-1 flex h-5 w-5 items-center justify-center rounded-full text-gray-300 transition-colors`}
    >
      <FaStar
        className={`h-5 w-5 transition-all duration-200 ease-in-out ${
          starredItems.includes(String(item.id))
            ? "text-zik-main scale-110"
            : "hover:text-zik-main/80 text-gray-300"
        }`}
      />
    </button>
  );
...
export const ScrollGuide = () => (
  <div className="text-zik-text my-4 rounded-lg py-2 text-center text-sm">
