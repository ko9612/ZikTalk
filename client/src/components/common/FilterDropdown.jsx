...
const FilterDropdown = ({ value = { type: '최신순', rating: 0 }, onChange }) => {
  // 드롭다운 열림/닫힘 상태 관리
    ...
    <div className="relative w-48" ref={ref}>
      ...
      >
        ...
        <span className={`ml-2 transition-transform duration-200 ${open ? 'rotate-180' : ''}`}>
          <svg width="10" height="10" viewBox="0 0 20 13" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {open && (
        <div 
          className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg animate-fadeIn"
            ...
            {options.map((opt) => (
              <li
                ...
                className={`px-4 py-2 text-sm cursor-pointer hover:bg-gray-100 ${
                  displayValue.type === opt.value ? 'bg-gray-50 text-zik-main font-medium' : 'text-gray-700'
                }`}
